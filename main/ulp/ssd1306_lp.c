...
static uint8_t oled_buffer[512]; //Can change 32x128px-512bytes, 64x128px-1024b, 128x128px-2048b
...
void oled_lp_set_pump(oled_pump_volt_t voltage){} // This only for SSD1312
...
void oled_lp_power_on(void) {
    ...
    ulp_lp_core_delay_us(25000); oled_send_command(0xAF); 
}

void oled_lp_init(void) {
    oled_lp_time_init();
    oled_send_command(0xE3);
    oled_send_command(0xAE); 
    oled_send_command(0xD5); oled_send_command(0x80); 
    oled_send_command(0xA8); oled_send_command(0x1F); // Mux Ratio: 0x3F (63) for 64px height, 0x1F (31) for 32px height
    oled_send_command(0x40); 
    oled_send_command(0x8D); oled_send_command(0x14);
    oled_send_command(0x20); oled_send_command(0x02); 
    oled_send_command(0xA1); 
    oled_send_command(0xC0); 
    oled_send_command(0xDA); oled_send_command(0x12); // COM Pins: 0x12 or 0x02
    oled_send_command(0xD3); oled_send_command(0x00);
    
    oled_send_command(0x81); oled_send_command(0xFF); //Base bright 00-FF
    oled_send_command(0xD9); oled_send_command(0x22);
    oled_send_command(0xDB); oled_send_command(0x20); // Can be changed to 0x30 or 0x10 or 0x00
    
    oled_send_command(0x2E); 
    oled_send_command(0xA4); 
    oled_send_command(0xA6); 
    oled_send_command(0xAC);
...
void oled_lp_refresh_once(void) {
    ...
    for (uint8_t page = 0; page < 4; page++) { // Can change this 32px - 4 page, 64px - 8page, 128px - 16page
        oled_send_command(0xB0 + page); // Page start address
        oled_send_command(0x00); oled_send_command(0x10);
